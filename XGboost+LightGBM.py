...
def kfold_lightgbm(train_df, test_df, num_folds, stratified = False, debug= False):
    ...
    ENSEMBLE_NUM = 2
    ensemble_preds = np.zeros(test_df.shape[0])
    for nnn in range(ENSEMBLE_NUM):
        ...
        for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['outliers'])):
            ...
            params ={
                    ...
                    'seed':int((nnn+1)*2**n_fold),
                    'bagging_seed':int((nnn+1)*2**n_fold),
                    'drop_seed':int((nnn+1)*2**n_fold)
                    }
        ...
        ensemble_preds += sub_preds / ENSEMBLE_NUM
    ...
    xmodel_rand_seed = [123, 4424]
    ensem2_preds = np.zeros(test_df.shape[0])
    for nnn in range(len(xmodel_rand_seed)):
        xmodel = XGBRegressor(base_score=0.5, booster='gbtree', colsample_bylevel=1,
               colsample_bytree=1, gamma=0, learning_rate=0.02, max_delta_step=0,
               max_depth=6, min_child_weight=316.22776601683796, missing=None,
               n_estimators=1010, n_jobs=4, nthread=None, objective='reg:linear',
               random_state=0, reg_alpha=562.341325190349, reg_lambda=1,
               scale_pos_weight=1, seed=123, silent=True, subsample=1)
        xmodel.fit(train_df[feats], train_df['target'], eval_metric = 'rmse') #Fit the algorithm on the data
        train_pred = xmodel.predict(train_df[feats])
        print("RMSE (Train): ", np.sqrt(metrics.mean_squared_error(train_df['target'], train_pred)))
        test_pred = xmodel.predict(test_df[feats])
        
        ensem2_preds += test_pred / len(xmodel_rand_seed)
    ...
    if not debug:
        ...
        # test_df.loc[:,'target'] = ensemble_preds
        test_df.loc[:,'target'] = (ensemble_preds + ensem2_preds) / 2
